...
void ChunkSort(const int n, const int m, const int P, int *array) {
  ...
  ThreadInfo info = {
      ...
      .m = m,
      .array  = array,
      .chunks_to_sort = chunks_per_thread,
      .begin = 0
  };
  ...
  for (size_t thread_number = 0; thread_number < P;
    ...
    info.begin = array_index;
    pthread_create(&(threads[thread_number]), NULL, SortSomeChunks, &info);
  }
  ...
  while (size < m) {
    ...
    while (array_index + size < n - 1) {
      ...
      for (int i = 0; i < P; ++i) {
        ...
        MergeInfo info = {
            .first_left = array_index,
            .first_right = array_index + size - 1,
            .second_left = array_index + size,
            .second_right = MIN(array_index + 2 * size, n) - 1,
            .array = array,
        };
        array_index = info.second_right + 1;
        pthread_create(&(threads[i]), NULL, MergeChunks, &info);
      }
...
int main(int argc, char** argv) {
  ...
  fprintf(stats, "%.4fs %d %d %d\n", parallel_sort_time, n, m, P);
  fclose(stats);

  FILE* builtin_stats = fopen("builtin_stats.txt", "w");
  fprintf(builtin_stats, "%.4fs %d\n", builtin_sort_time, n);
  fclose(builtin_stats);
